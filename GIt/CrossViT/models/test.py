    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384),
        ...
        if hybrid_backbone is None:  #修改，是有差异的，一个是列表，另一个是元组的形式进行存储
            self.pos_embed = ms.ParameterTuple(
                [ms.Parameter(ms.ops.Zeros(1, 1 + num_patches[i], embed_dim[i])) for i in range(self.num_branches)])
        ...
        self.cls_token = nn.ParameterList(
            [nn.Parameter(ms.ops.Zeros(1, 1, embed_dim[i])) for i in range(self.num_branches)])
        self.pos_drop = nn.Dropout(p=drop_rate)

        total_depth = sum([sum(x[-2:]) for x in depth])
        dpr = [x.item() for x in ms.ops.LinSpace(0, drop_path_rate, total_depth)]  # stochastic depth decay rule
        dpr_ptr = 0
